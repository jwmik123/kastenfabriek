...
export default function GTAOEffect() {
  ...
  // This is a common pattern to "take over" the render loop in R3F
  return () => { gl.render = previousRenderMode }
}, [gl])

  useFrame(({ camera }) => {
    // GTAONode snapshots the camera matrices at init via uniform(). R3F updates the
    // camera's projection matrix in-place on resize, but Three.js WebGPU doesn't
    // auto-detect Matrix4 mutations — so we push the current values each frame.
    // Manual update of AO uniforms as you already have
